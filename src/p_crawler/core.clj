...
(defn crawl-web [seed]
  (onto-chan uri-queue seed false)
  (doseq [thread-index (range 8)]
    (thread (swap! running-threads inc)
            (loop [url (<!! uri-queue)]
              (when url
                (process-url url)
                (recur (<!! uri-queue))))
            (swap! running-threads dec) 
            (info (str "Download thread " thread-index " has ended.")))))
